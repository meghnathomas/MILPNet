...
#                                print('yo')
    ...
    v1, v2, v3 = {}, {}, {}    
    y_pumps_proxy, diff_y, consecutive_y = {}, {}, {}                                   # valve status binary variables
...
#                        print(link_ind)
    ...
    #        
        ...
        diurnal_cost_function_30min = [off_peak * mean_power] * 14 + [mid_peak * mean_power] * 16 + [on_peak * mean_power] * 6 + [mid_peak * mean_power] * 8 + [off_peak * mean_power] * 4
        
        reverse_diurnal_cost_function_30min = [on_peak * mean_power] * 14 + [mid_peak * mean_power] * 16 + [off_peak * mean_power] * 6 + [mid_peak * mean_power] * 8 + [on_peak * mean_power] * 4
       
       # 2 hr
       
        diurnal_cost_function_2hr = [off_peak * mean_power] * 3 + [mid_peak * mean_power] * 4 + [on_peak * mean_power] * 3 + [mid_peak * mean_power] * 4 + [off_peak * mean_power] * 2
       
        reverse_diurnal_cost_function_2hr = [on_peak * mean_power] * 3 + [mid_peak * mean_power] * 4 + [off_peak * mean_power] * 3 + [mid_peak * mean_power] * 4 + [on_peak * mean_power] * 2
       
        if ob_fn == 'static':
            cost_function = static_cost_function
            
        if ob_fn == 'diurnal':
            cost_function = diurnal_cost_function
       
        if ob_fn == 'reverse':
            cost_function = reverse_diurnal_cost_function 
            
        if ob_fn == 'diurnal_30min':
            cost_function = diurnal_cost_function_30min
       
        if ob_fn == 'reverse_30min':
            cost_function = reverse_diurnal_cost_function_30min 
            
        if ob_fn == 'diurnal_2hr':
            cost_function = diurnal_cost_function_2hr
       
        if ob_fn == 'reverse_2hr':
            cost_function = reverse_diurnal_cost_function_2hr 
       
        # Define objective function
        obj = gp.quicksum(cost_function[i] * y_pumps_proxy[i][j] for i in range(no_ts) for j in range(num_pumps)) + gp.quicksum(penalties[i] * diff_y[i][j] for i in range(no_ts-1) for j in range(num_pumps))
